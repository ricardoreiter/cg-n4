...
public abstract class WorldObject implements Drawable, Updatable, PhysicsUpdatable {
	...
	public WorldObject(GraphicObject graphicObject, CollisionShape collisionShape, Vector3f initPos) {
		...
		RigidBodyConstructionInfo rigidBodyInfo = new RigidBodyConstructionInfo(0, this.motionState,
				this.collisionShape, new Vector3f(0, 0, 0));
	...
	public void changeColor(float[] color) {
		graphicObject.changeMaterialColor(color);
	}

	public void setSize(Vector3f size) {
		if (rigidBody != null) {
			throw new UnsupportedOperationException(
					"Mudar o tamanho de um objeto com corpo rígido depois de criado por enquanto não é possível");
		}
